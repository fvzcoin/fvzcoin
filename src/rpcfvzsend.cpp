...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (strCommand == "count") return (int)fvzSendMasterNodes.size();
    ...
    if (strCommand == "current")
    {
        ...
        int winner = fvzSendPool.GetCurrentMasterNode(mod);
        if(winner >= 0) {
            return fvzSendMasterNodes[winner].addr.ToString().c_str();
        }
    ...
    {
        Object obj;
        BOOST_FOREACH(CMasterNodeVote& mv, fvzSendMasterNodeVotes) {
