...
uint256 hashGenesisBlock("0x000005b25af09f023520e4e5904248a5efe8d471d7a2788b5c484bf0bab0d33f"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // FVZCoin: starting difficulty is 1 / 2^12
...
// create FVZSend pools
CFVZSendPool fvzSendPool;
CFVZSendSigner fvzSendSigner;
std::vector<CMasterNode> fvzSendMasterNodes;
std::vector<CMasterNodeVote> fvzSendMasterNodeVotes;
...
const string strMessageMagic = "FVZCoin Signed Message:\n";
...
{
    // FVZCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // FVZCoin: 1 day
static const int64 nTargetSpacing = 1 * 60; // FVZCoin: 1 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // FVZCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static FVZGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, fvzcoin - darkGravity v2, written by Evan Duffield - evan@darkcoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static FVZGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, fvzcoin - DARKGravity v3, written by Evan Duffield - evan@darkcoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
            ...
            else if (pindexLast->nHeight+1 >= 20) { DiffMode = 3; }
            else if (pindexLast->nHeight+1 >= 10) { DiffMode = 2; }
        }

        if (DiffMode == 1) { return GetNextWorkRequired_V1(pindexLast, pblock); }
        else if (DiffMode == 2) { return GetNextWorkRequired_V2(pindexLast, pblock); }
        else if (DiffMode == 3) { return FVZGravityWave(pindexLast, pblock); }
        else if (DiffMode == 4) { return FVZGravityWave3(pindexLast, pblock); }
        return FVZGravityWave3(pindexLast, pblock);
}
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
    ...
    if ((pindexNew->nHeight % 300) == 0 || (!fIsInitialDownload && (pindexNew->nHeight % 144) == 0))
    {
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // FVZCoin: Special short-term limits to avoid 10,000 BDB lock limit:
    ...
    if(MasternodePayments && EnforceMasternodePayments)
    {
        ...
        int64 masternodePaymentAmount = 0;
        bool fIsInitialDownload = IsInitialBlockDownload();
...
{
    // FVZCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        hashGenesisBlock = uint256("0x0000059ff4ae4152dabf0c9f3554cc179e90d97dcfa003d60cdbba3c1919b03c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "FVZ 09/Jul/2014 No Borders No Notions FVZ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1404993847;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1520231;

        if (fTestNet)
        {
            block.nTime    = 1404989919;
            block.nNonce   = 2277302;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xc38e19033fca17259287a6ec79f67a3db8c095e55ba8e4c0636bdcfeb064a06f"));
         block.print();
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (pfrom->nVersion >= fvzSendPool.MIN_PEER_PROTO_VERSION) {
    ...
    else if (strCommand == "verack")
    {
    ...
    else if (strCommand == "dseg") { //FVZSend Election Get
        if (pfrom->nVersion < fvzSendPool.MIN_PEER_PROTO_VERSION) {
            return false;
        }

        int count = fvzSendMasterNodes.size()-1;
        int i = 0;

        BOOST_FOREACH(CMasterNode mn, fvzSendMasterNodes) {
    ...
    else if (strCommand == "dsee") { //FVZSend Election Entry
        if (pfrom->nVersion < fvzSendPool.MIN_PEER_PROTO_VERSION) {
            return false;
        }
        bool fIsInitialDownload = IsInitialBlockDownload();
        ...
        if(!fvzSendSigner.VerifyMessage(pubkey, vchSig, strMessage, errorMessage)){
        ...
        BOOST_FOREACH(CMasterNode& mn, fvzSendMasterNodes) {
            ...
            if(mn.vin == vin) {
                if(!mn.UpdatedWithin(MASTERNODE_MIN_MICROSECONDS)){
                    ...
                    if(count == -1)
                        RelayFVZSendElectionEntry(vin, addr, vchSig, sigTime, pubkey, pubkey2, count, current, lastUpdated);
                }
        ...
        CTxOut vout = CTxOut(999.99*COIN, fvzSendPool.collateralPubKey);
        ...
        if(tx.AcceptableInputs(state, true)){
            ...
            fvzSendMasterNodes.push_back(mn);
    ...
    else if (strCommand == "dseep") { //FVZSend Election Entry Ping
        if (pfrom->nVersion < fvzSendPool.MIN_PEER_PROTO_VERSION) {
            return false;
        }
        bool fIsInitialDownload = IsInitialBlockDownload();
        ...
        BOOST_FOREACH(CMasterNode& mn, fvzSendMasterNodes) {
            ...
            if(mn.vin == vin) {
                ...
                std::string errorMessage = "";
                if(!fvzSendSigner.VerifyMessage(mn.pubkey2, vchSig, strMessage, errorMessage)){
                ...
                if(stop) {
                    if(mn.IsEnabled()){
                        mn.Disable();
                        mn.Check();
                        RelayFVZSendElectionEntryPing(vin, vchSig, sigTime, stop);
                    }
                    return true;
    ...
    int64 nFees = 0;
    {
        ...
        if(bMasterNodePayment) {
            if(!pblock->MasterNodePaymentsEnforcing()){
                int winningNode = fvzSendPool.GetCurrentMasterNode(1);
                if(winningNode >= 0){
                    pblock->payee.SetDestination(fvzSendMasterNodes[winningNode].pubkey.GetID());

                    payments++;
                    txNew.vout.resize(payments);

                    //txNew.vout[0].scriptPubKey = scriptPubKeyIn;
                    txNew.vout[payments-1].scriptPubKey.SetDestination(fvzSendMasterNodes[winningNode].pubkey.GetID());
                ...
                if(blockLast.ReadFromDisk(pindexPrev)){
                    BOOST_FOREACH(CMasterNodeVote& mv1, blockLast.vmn){
                        ...
                        if(mv1.GetVotes() >= MASTERNODE_PAYMENTS_MIN_VOTES-1){
                            ...
                            BOOST_FOREACH(CMasterNodeVote& mv2, fvzSendMasterNodeVotes) {
                                if((mv1.blockHeight == mv2.blockHeight && mv1.GetPubKey() == mv2.GetPubKey())) {
                                    mv1.Vote();
                                    break;
                                }
                ...
                if(winningNode >= 0){
                    CMasterNodeVote mv;
                    mv.Set(fvzSendMasterNodes[winningNode].pubkey, pindexPrev->nHeight + 1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FVZCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FVZCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FVZCoinMiner(CWallet *pwallet)
{
    printf("FVZCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fvzcoin-miner");
    ...
    try { loop {
        ...
        printf("Running FVZCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FVZCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FVZCoinMiner, pwallet));
}
...
bool CFVZSendPool::SetCollateralAddress(std::string strAddress){
      CBitcoinAddress address;
      if (!address.SetString(strAddress))
   {
          printf("CFVZSendPool::SetCollateralAddress - Invalid FVZSend collateral address\n");
...
bool CFVZSendPool::GetLastValidBlockHash(uint256& hash, int mod)
{
...
void CFVZSendPool::NewBlock()
{
    if(fDebug) printf("CFVZSendPool::NewBlock \n");

    {
        ...
        if(pindexBest != NULL) {
            ...
            if(winningNode >= 0){
                CMasterNodeVote mv;
                mv.Set(fvzSendMasterNodes[winningNode].pubkey, pindexBest->nHeight + 1);
                fvzSendMasterNodeVotes.push_back(mv);

                if(fvzSendMasterNodeVotes.size() > MASTERNODE_PAYMENTS_EXPIRATION){
                    fvzSendMasterNodeVotes.erase(fvzSendMasterNodeVotes.begin(), fvzSendMasterNodeVotes.end()-MASTERNODE_PAYMENTS_EXPIRATION);
                }
            }
        }
    }
...
uint256 CMasterNode::CalculateScore(int mod)
{
    ...
    uint256 n1 = 0;
    if(!fvzSendPool.GetLastValidBlockHash(n1, mod)) return 0;
...
int CFVZSendPool::GetCurrentMasterNode(int mod)
{
    int i = 0;
...
bool CFVZSendSigner::SetKey(std::string strSecret, std::string& errorMessage, CKey& key, CPubKey& pubkey){
...
bool CFVZSendSigner::SignMessage(std::string strMessage, std::string& errorMessage, vector<unsigned char>& vchSig, CKey key)
{
...
bool CFVZSendSigner::VerifyMessage(CPubKey pubkey, vector<unsigned char>& vchSig, std::string strMessage, std::string& errorMessage)
{
    CHashWriter ss(SER_GETHASH, 0);
    ss << strMessageMagic;
    ss << strMessage;
